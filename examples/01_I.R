...
y <- 4;
x <- 4;
(angle1 = atan(y / x) * 180 / pi + 360);
(angle2 = atan2(y, x) * 180 / pi + 360);
...
plot(c(1,800), c(1,900), type="n", axes=F,xlab="", ylab="",cex.lab=1.5, cex.axis=1.5,font.main = 1);
title("Cluster Number",cex.main = 4, line = -15, font.main = 1)
  ...
  if (i > 1){
    for (clu.i in 1:clu.m){
      ...
      for (p.i in p.c){
        ...
        x0 <- oldx[p.i];
        y0 <- oldy[p.i];
        segments(x0, y0, x1, y1, col=col0[7], lwd=0.6);
      }
    }
  }
  oldx <- out$x;
